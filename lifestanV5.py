...
ADMIN_IDS = [int(x) for x in os.getenv('ADMIN_IDS', '8473394476').split(',')]
MAX_FREE_REQUESTS = 3
...
# ПОВЫШЕННЫЕ ЦЕНЫ
PHONE_CHECK_PRICE = 30  # было 1
EMAIL_CHECK_PRICE = 20  # было 2
...
RUSSIAN_REGIONS = {
    ...
    '493': 'Костромская область', '494': 'Ивановская область',
...
def daily_bonus(message):
    ...
    if result and result[0] == today:
        bot.send_message(
            ...
            "🔄 Приходите завтра для получения нового бонуса.",
            reply_markup=create_bonus_menu()
        )
        conn.close()
        return
    
    # Выдаем случайный бонус (10-50 коинов)
    bonus_amount = random.randint(10, 50)
    
    # Обновляем баланс и записываем дату получения
    ...
    c.execute("INSERT INTO transactions (user_id, amount, type, description) VALUES (?, ?, ?, ?)",
              (user_id, bonus_amount, "daily_bonus", "Ежедневный бонус"))
    ...
    bot.send_message(
        ...
        f"💰 *Получено:* {bonus_amount} {CURRENCY_NAME}\n"
...
@bot.message_handler(func=lambda msg: msg.text == "👥 Реферальная система")
def referral_system(message):
    ...
    response = (
        ...
        f"• 💰 Заработано бонусов: {total_bonus} {CURRENCY_NAME}\n\n"
        f"🎁 *Как это работает:*\n"
        f"1. Поделитесь своей реферальной ссылкой\n"
        f"2. За каждого приглашенного друга вы получаете {REFERRAL_BONUS} {CURRENCY_NAME}\n"
        f"3. Ваш друг получает 100 {CURRENCY_NAME} при регистрации\n\n"
...
def activate_promo_process(message):
    ...
    if message.text == "⬅️ Назад":
        bot.send_message(message.chat.id, "Меню бонусов:", reply_markup=create_bonus_menu())
        return
    ...
    try:
        ...
        if not promo:
            bot.send_message(
                message.chat.id,
                "❌ *Промокод не найден!*\n\n"
                "Проверьте правильность ввода или срок действия промокода.",
                parse_mode="Markdown",
                reply_markup=create_bonus_menu()
            )
            conn.close()
        ...
        if is_active == 0:
            bot.send_message(message.chat.id, "❌ Промокод деактивирован!", reply_markup=create_bonus_menu())
            conn.close()
            return
        ...
        if expiry_date:
            ...
            if expiry < datetime.now():
                bot.send_message(message.chat.id, "❌ Срок действия промокода истек!", reply_markup=create_bonus_menu())
                conn.close()
                return
        
        # Проверяем количество использований
        if uses_left <= 0:
            bot.send_message(message.chat.id, "❌ Промокод уже использован максимальное количество раз!", reply_markup=create_bonus_menu())
            conn.close()
            return
        ...
        if c.fetchone():
            bot.send_message(message.chat.id, "❌ Вы уже активировали этот промокод!", reply_markup=create_bonus_menu())
            conn.close()
            return
        
        # Обновляем баланс пользователя
        update_balance(user_id, amount, "promocode", f"Активация промокода: {promo_code}")
        ...
        bot.send_message(
            message.chat.id,
            f"✅ *Промокод активирован успешно!*\n\n"
            f"🎫 *Промокод:* {promo_code}\n"
            f"💰 *Получено:* {amount} {CURRENCY_NAME}\n"
            f"💳 *Новый баланс:* {new_balance} {CURRENCY_NAME}",
            parse_mode="Markdown",
            reply_markup=create_bonus_menu()
        )
        ...
        bot.send_message(message.chat.id, f"❌ Ошибка: {str(e)}", reply_markup=create_bonus_menu())
    finally:
        conn.close()
...
def give_coins_start(message):
    msg = bot.send_message(
        ...
        "Введите ID пользователя и количество коинов через пробел:\n"
...
def give_coins_process(message):
    ...
    try:
        ...
        if len(parts) != 2:
            bot.send_message(message.chat.id, "❌ Неверный формат. Используйте: ID КОЛИЧЕСТВО")
            return
        ...
        if amount <= 0:
            bot.send_message(message.chat.id, "❌ Количество коинов должно быть положительным")
            return
        ...
        # Выдаем коины
        update_balance(user_id, amount, "admin_gift", "Выдача коинов администратором")
        
        # Отправляем уведомление пользователю
        try:
            bot.send_message(
                user_id,
                f"🎁 *Вам выданы коины администратором!*\n\n"
                f"💰 *Получено:* {amount} {CURRENCY_NAME}\n"
                f"💳 *Новый баланс:* {get_user_balance(user_id)} {CURRENCY_NAME}",
                parse_mode="Markdown"
            )
        except:
            pass
        ...
        bot.send_message(
            ...
            f"🆔 *ID:* {user_id}\n"
            f"💰 *Выдано:* {amount} {CURRENCY_NAME}",
            parse_mode="Markdown",
            reply_markup=create_admin_menu()
        )
        
    except ValueError:
        bot.send_message(message.chat.id, "❌ Ошибка в данных. Проверьте ID и количество коинов")
    except Exception as e:
...
def create_promo_start(message):
    msg = bot.send_message(
        ...
        "`КОЛИЧЕСТВО_КОИНОВ КОЛИЧЕСТВО_АКТИВАЦИЙ ОПИСАНИЕ`\n\n"
        "Пример: `100 10 Праздничный промокод`\n\n"
        "Примечание:\n"
        "- Количество активаций: 0 = безлимитно",
        reply_markup=create_back_button(),
        parse_mode="Markdown"
    )
...
def create_promo_process(message):
    ...
    try:
        ...
        if len(parts) < 3:
            bot.send_message(message.chat.id, "❌ Неверный формат. Используйте: КОИНЫ АКТИВАЦИИ ОПИСАНИЕ")
            return
        ...
        uses_text = "безлимитно" if uses_total == 0 else f"{uses_total} активаций"
        
        bot.send_message(
            message.chat.id,
            f"✅ *Промокод создан успешно!*\n\n"
            f"🎫 *Код:* `{promo_code}`\n"
            f"💰 *Коины:* {amount} {CURRENCY_NAME}\n"
        ...
        bot.send_message(message.chat.id, "❌ Ошибка в данных. Проверьте формат ввода")
    except Exception as e:
...
def list_promocodes(message):
    ...
    response = "📋 *Последние 20 промокодов*\n\n"
    
    for promo in promos:
        code, amount, uses_total, uses_left, expiry_date, is_active, description = promo
        status = "✅ Активен" if is_active == 1 else "❌ Неактивен"
        uses_text = f"{uses_left}/{uses_total}" if uses_total > 0 else "∞"
        
        response += f"🎫 *{code}* - {status}\n"
        response += f"💰 {amount} коинов | 📊 {uses_text} использований\n"
        if expiry_date:
            response += f"📅 Действует до: {expiry_date}\n"
...
def manage_users(message):
    ...
    for user in users:
        ...
        response += f"{premium_status}{ban_status} *{first_name or 'Без имени'}* (@{username or 'нет'})\n"
        response += f"🆔 `{user_id}` | 💰 {balance} коинов\n"
        response += f"{'-'*30}\n"
    
    markup = types.InlineKeyboardMarkup()
    markup.add(
        types.InlineKeyboardButton("📊 Подробная статистика", callback_data="admin_detailed_stats"),
        types.InlineKeyboardButton("📈 Графики", callback_data="admin_charts")
    )
...
def bot_stats(message):
    ...
    response = (
        ...
        f"• Коинов в системе: {total_coins}\n\n"
        f"📅 *Активность за 7 дней:*\n"
    )
    
    for date_str, count in daily_stats[:7]:  # Последние 7 дней
        response += f"• {date_str}: {count} запросов\n"
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("📤 Экспорт", callback_data="export_stats")
    )
...
@bot.message_handler(func=lambda msg: msg.text == "⚙️ Настройки" and is_admin(msg.from_user.id))
def admin_settings(message):
    response = (
        f"⚙️ *Настройки бота*\n\n"
        f"📊 *Текущие настройки:*\n"
        f"• Стоимость проверки номера: {PHONE_CHECK_PRICE} {CURRENCY_NAME}\n"
        f"• Стоимость проверки email: {EMAIL_CHECK_PRICE} {CURRENCY_NAME}\n"
        f"• Бонус за реферала: {REFERRAL_BONUS} {CURRENCY_NAME}\n"
        f"• Начальный баланс: 100 {CURRENCY_NAME}\n"
        f"• Премиум 1 день: {PREMIUM_COST_1DAY} {CURRENCY_NAME}\n"
        f"• Премиум 7 дней: {PREMIUM_COST_7DAY} {CURRENCY_NAME}\n"
        f"• Премиум 30 дней: {PREMIUM_COST_30DAY} {CURRENCY_NAME}\n\n"
        f"📢 *Канал:* {CHANNEL_USERNAME}\n"
        f"🔐 *Админы:* {len(ADMIN_IDS)} человек\n\n"
        f"*Для изменения настроек отредактируйте конфигурацию в коде бота.*"
    )
...
def ban_user_process(message):
    ...
    try:
        ...
        bot.send_message(
            message.chat.id,
            f"{action_emoji} *Пользователь {action}!*\n\n"
            f"👤 *Пользователь:* {first_name} (@{username})\n"
            f"🆔 *ID:* {user_id}",
            parse_mode="Markdown",
            reply_markup=create_admin_menu()
...
def give_premium_start(message):
    msg = bot.send_message(
        ...
        "Пример: `123456789 30`\n\n"
        "Для отмены премиума укажите 0 дней",
        reply_markup=create_back_button(),
        parse_mode="Markdown"
    )
...
def give_premium_process(message):
    ...
    try:
        ...
        if days == 0:
            ...
            action = "выдан"
            action_emoji = "✅"
            expiry_text = f"до {new_expiry.strftime('%d.%m.%Y %H:%M')}"
        ...
        try:
            if days == 0:
                bot.send_message(
                    user_id,
                    f"❌ *Ваш премиум статус отменен администратором!*",
                    parse_mode="Markdown"
                )
            else:
                bot.send_message(
                    user_id,
                    f"🎉 *Вам выдан премиум администратором!*\n\n"
                    f"💎 *Срок:* {days} дней\n"
        ...
        response = f"{action_emoji} *Премиум {action}!*\n\n"
        response += f"👤 *Пользователь:* {first_name} (@{username})\n"
        response += f"🆔 *ID:* {user_id}\n"
        
        if days > 0:
            response += f"💎 *Срок:* {days} дней\n"
            response += f"📅 *Действует:* {expiry_text}"
        else:
            response += f"💎 *Статус:* Премиум отменен"
...
# --- ОБРАБОТЧИКИ ДЛЯ ОСТАЛЬНЫХ ФУНКЦИЙ ---
...
def process_phone_number(message):
    ...
    bot.send_message(message.chat.id, "🔍 Проверяю номер...")
...
def admin_detailed_stats(call):
    ...
    response += f"• Всего: {total_users}\n"
    response += f"• Премиум: {premium_users} ({premium_users/total_users*100:.1f}%)\n"
    response += f"• Заблокировано: {banned_users}\n"
    response += f"• С рефералами: {users_with_refs}\n\n"
    
    response += f"💰 *Экономика:*\n"
    response += f"• Всего коинов: {total_coins or 0}\n"
    response += f"• Средний баланс: {avg_coins or 0:.0f}\n\n"
    
    response += f"📈 *Активность (последние 7 дней):*\n"
    for date_str, count in weekly_activity:
        response += f"• {date_str}: {count} запросов\n"
    
    response += f"\n🏆 *Топ-5 по балансу:*\n"
    for i, (uid, username, name, balance) in enumerate(top_users, 1):
        response += f"{i}. {name or 'Без имени'} (@{username or 'нет'}) - {balance} коинов\n"
...
def refresh_stats(call):
    # Просто пересылаем команду статистики
    message = types.Message.de_json(call.message.json)
    message.text = "📊 Статистика бота"
